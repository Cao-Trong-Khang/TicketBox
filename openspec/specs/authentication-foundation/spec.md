...
The system SHALL allow a new user account to register through the Backend API. This capability supports future Audience, Organizer, and Check-in Staff authentication, but it MUST NOT assign roles or permissions in this change.
...
6. The Backend API returns the created user identity without exposing `password_hash`.
...
- Registration MUST NOT create records in `roles`, `permissions`, `user_roles`, or `role_permissions`.
