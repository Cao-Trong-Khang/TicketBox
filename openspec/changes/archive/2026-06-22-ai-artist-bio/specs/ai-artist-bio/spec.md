## Specification: Asynchronous AI Artist Biography
...
1. The Organizer Web Application validates and uploads a PDF to the Backend API.
2. The Backend API authenticates the Organizer, checks permission and concert ownership, stores the PDF in MinIO, persists `uploaded` state in PostgreSQL, and publishes `ai.bio.requested` to Kafka.
3. A Background Worker consumes the event, updates durable status, downloads and extracts the PDF, stores cleaned text, and invokes the configured AI Model adapter with at most 4,000 characters.
4. The worker stores the completed biography and document state in PostgreSQL and invalidates the Redis concert-detail cache.
5. The Organizer Web Application polls status and allows regeneration or manual editing.
6. The Audience Web Application receives `artist_bio` only through public concert detail when a completed biography exists.

## Failure Scenarios

- Invalid type/size returns 400 before event publication; unauthorized or non-owner access returns 401/403 without mutation.
- MinIO timeout receives bounded exponential retries; unusable extracted text fails permanently without AI invocation.
- AI timeout and rate limiting follow their distinct bounded retry/delay policy, then persist a safe failure reason.
- Kafka publish failure prevents a false 202 response and leaves recoverable durable state.
- Worker crash before offset commit causes Kafka redelivery and idempotent processing.
- Redis invalidation failure can temporarily leave cached content stale, but cache TTL and PostgreSQL remain the recovery/source-of-truth path.

## Constraints

- PostgreSQL is authoritative; Redis MUST NOT hold final document or biography state.
- Admin actions require Organizer role/permission plus Layer 2 concert ownership and document-concert consistency checks.
- Object storage is private, storage keys are server-generated, PDFs are at most 10 MB, extraction under 50 characters fails, and AI input is at most 4,000 cleaned characters.
- Kafka processing is at-least-once and MUST be idempotent. The offset is not committed before a durable terminal outcome.
- AI/provider failures MUST NOT break public concert APIs, checkout, payments, VIP import, notifications, or check-in.
- New endpoint errors follow `{ error, message, status_code }` and do not leak internal/provider data.

## Acceptance Criteria

- Automated API tests verify valid upload/202, invalid file/400, missing auth/401, non-Organizer/403, non-owner/403, listing/detail scoping, manual edit, and regeneration with a new document id.
- Worker tests verify ordered status changes, text cleaning and the 50-character threshold, the 4,000-character prompt cap, each retry policy, permanent extraction failure, duplicate event handling, and crash/redelivery-safe persistence.
- Integration tests with PostgreSQL, Kafka, MinIO, Redis, and the mock AI adapter demonstrate an upload progressing to `done` while the initial request returns before generation completes.
- Public API tests verify the field is absent for no bio, processing, and failed-only states; present for the newest completed state; and updated after manual edit/cache invalidation.
- Frontend tests verify 10 MB/PDF validation, 3–5 second polling, terminal-state stop, failure display, regeneration, editing, and optional public rendering.
